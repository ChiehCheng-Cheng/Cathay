...
def ask_insurance_question(user_input, qa_top_n=5, km_top_n=5):
    ...
    請判斷以下「使用者輸入」的意圖是否明確，是否足以進行保險條文的精準檢索。
    使用者輸入：{user_input}
    
    規則：
    1.日常閒聊 (CHITCHAT)：如果是單純的打招呼、道謝、閒聊，或完全無關保險的字眼（例如：「早安」、「你好」、「謝謝」），請只輸出：CHITCHAT
    2.如果包含明確的問題、情境或動詞（例如：「班機延誤四小時有賠嗎？」、「住院理賠要附什麼文件」），請只輸出：CLEAR
    3.如果只有簡短名詞、模糊字眼，或是意圖不明確的片段（例如：「住院」、「延誤」、「行李」、「理賠」），請只輸出：UNCLEAR
    
    請絕對只輸出 CHITCHAT、CLEAR 或 UNCLEAR。
    """
    
    intent_decision = llm.invoke([("human", intent_check_prompt)]).content.strip().upper()
    print(f"[意圖理解] 判斷結果: {intent_decision}")

    # ==========================================
    # 【路線 C：日常閒聊】直接由 LLM 生成友善回覆
    # ==========================================
    if "CHITCHAT" in intent_decision:
        print("[閒聊機制] 啟動：單純打招呼或閒聊，直接生成回覆...")
        
        chitchat_prompt = f"""
        使用者對您說：「{user_input}」
        
        請您扮演「國泰產險智能助理阿發」，給予親切且簡短的回覆。
        回覆必須包含兩個要素：
        1. 針對使用者的話給予適當的回應（例如早安、不客氣等）。
        2. 主動詢問您是否有關於「海外旅行不便險」的理賠問題需要協助。
        3. 轉換為一般使用者口吻：請務必模擬一般人自然語氣回復。**絕對不要**在問題中出現任何「條款編號」、「依據第X條」等法律字眼。
        4. 禁用引用標記：絕對禁止在選項中使用 `` 或任何方括號標記！
        
        注意：請務必使用「您」來稱呼使用者。
        """
        
        chitchat_answer = llm.invoke([
            ("system", system_prompt), 
            ("human", chitchat_prompt)
        ]).content.strip()
        ...
        return {
         ...
         # ==========================================
     # 【路線 B：追問機制】意圖不明確，進入 QA 庫檢索 Top N
     # ==========================================
    elif "UNCLEAR" in intent_decision:
        ...
        for i, doc in enumerate(clarify_docs):
            ...
            clarify_text += f"[參考條文 {i+1}]\n{c_text}\n\n"
        
        # 2. 輸出 Top N：讓 LLM 轉化為友善的追問選項
        clarify_prompt = f"""
        使用者的提問「{user_input}」過於簡短，意圖不明確。
        系統已經根據這個關鍵字，初步檢索出 {qa_top_n} 筆可能相關的保險條文：
        {clarify_text}
        
        請您根據這 {qa_top_n} 筆條文內容，生成 {qa_top_n} 個「具體且完整的問句」，作為提供給使用者的快捷點擊選項。
        
        ## 輸出格式與語氣規範（強制指令）
        1. **語意絕對一致（核心原則）**：生成的問句核心語意必須與對應的原始條文完全相同。您可以將用詞替換為白話文或貼近一般人的口吻，但「絕對不可以」自行擴充、縮減或改變條文原本設定的理賠條件、時數或情境。
        2. **轉換為一般使用者口吻**：請務必模擬一般人自然提問的語氣。**絕對不要**在問題中出現任何「條款編號」、「依據第X條」等法律字眼。
        3. **禁用引用標記**：絕對禁止在選項中使用 `` 或任何方括號標記！
        4. **聚焦情境**：將冷冰冰的條文轉換為使用者可能會遇到的真實情境問題。
        
        輸出格式範例：
        「您的提問較為簡短，請問您想了解的是以下哪一種情況呢？
        1. 請問班機延誤超過四小時的話，理賠的標準和金額怎麼算？
        2. 如果因為行程更改導致需要自己貼交通費，可以申請補償嗎？
        3. 請問在國外看病住院，回台灣要怎麼申請醫療費用的理賠？」
        """
        
        clarify_answer = llm.invoke([
            ("system", system_prompt), 
            ("human", clarify_prompt)
        ]).content.strip()
        
        # 抓取開頭的引言 (例如：「您的提問較為簡短...」)
        intro_text = clarify_answer.split("1.")[0].strip() if "1." in clarify_answer else "請問您想了解的是以下哪一種情況呢？"
        
        # 使用正規表達式抓取 1. 2. 3. 後面的具體問題
        options = re.findall(r'\d+\.\s*(.*)', clarify_answer)
        
        print(f"[輸出Top {qa_top_n}] 成功生成追問選項，並已轉換為按鈕陣列...")
        
        return {
            "answer": intro_text, # 只傳回引言作為對話泡泡文字
            "options": options,   # 傳回陣列，讓前端渲染按鈕 [Q1, Q2, Q3]
            "source": "系統追問選項",
            "type": "FOLLOW_UP_QUESTION" 
        }

     # ==========================================
     # 【路線 A：知識問答】意圖明確，進入 QA 庫檢索 Top N
     # ==========================================
    else:
        print("[知識問答] 啟動：意圖明確，進入標準檢索流程...")
        ...
        任務：
        請判斷這 {qa_top_n} 筆候選條款中，是否有一筆能「直接且完整並精準」地回答使用者的問題。
